...
* 원시타입과 객체간의 덧셈 속도는 차이가난다.
* 파이썬의 경우 객체로 구성되며 객체에서 수를 꺼내는 대만 해도 여러 연산을 거치며 여러 단계의 부가 작업이 필요하다.
* 원시 타입의 경우 메모리에서 값을 꺼내 한번 연산하면 끝나게 된다. 

### 자료구조 vs 자료형 vs 추상 자료형

#### 자료구조

* 컴퓨터과학에서 데이터에 효율적으로 접근하고 조직하기 위한 데이터의 조직, 관리, 저장 구조를 말한다.

#### 자료형

* 컴파일러 또는 인터프리터에게 프로그래머가 데이터를 어떻게 사용하는지를 알려주는 일종의 데이터 속성이다.
* 자료구조에 비해 더 구체적이다.
* 원시 타입까지 포함하게 된다.
